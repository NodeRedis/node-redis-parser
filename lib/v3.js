  ...
  while (offset < length) {
    ...
    if (c1 === 46) { // .
      ...
      if (res !== undefined) {
        ...
        return sign * (number + res / Math.pow(10, parser.offset - offset))
      }
      parser.offset = tmp
      return
    }
...
function reply (self, reply, push) {
  return function (data) {
    ...
    if (self.returnSet) {
      self.returnSet = false
      return push(new Set(data))
    }
    if (self.returnBlogError) {
      if (self.optionReturnBuffers === true) {
        data = data.toString()
      }
      const codeEnd = data.indexOf(' ')
      const code = data.slice(0, codeEnd)
      const err = new ReplyError(data.slice(codeEnd))
      err.code = code
      return reply(err)
    }
    if (self.returnMap) {
      self.returnMap = false
      const res = new Map()
      for (var i = 0; i < data.length; i += 2) {
        res.set(data[i], data[i + 1])
      }
      return reply(res)
    }
    return reply(data)
